...
int main(int argc, char *argv[])
{
	...
	if (setsockopt(fd, IPPROTO_SCTP, SCTP_NODELAY, &(int){1}, sizeof(int)) < 0) {
		printf("setsockopt SCTP_NODELAY %d (%s)\n", errno, strerror(errno));
		goto out;
	}
...
restart:
	...
	if (setsockopt(fd2, IPPROTO_SCTP, SCTP_NODELAY, &(int){1}, sizeof(int)) < 0) {
		printf("setsockopt SCTP_NODELAY %d (%s)\n", errno, strerror(errno));
		goto out;
	}
